...
export default async function UserManagementPage({
  ...
  searchParams: Promise<{ tab?: string; page?: string; q?: string; role?: string; status?: string }>
}) {
  const params = await searchParams
  const tab = params.tab === 'caregivers' ? 'caregivers' : 'people'
  ...
  return (
    <div className="p-2 space-y-1 mt-1">
      <AgencyUserManagementTabBar />

      {tab === 'people' && <AgencyPeopleTab agencyId={agencyId} />}
      {tab === 'caregivers' && caregiverContent}
    </div>
  )
}
